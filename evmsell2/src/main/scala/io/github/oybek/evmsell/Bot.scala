...
import io.github.oybek.evmsell.service.{DistanceCalculatorImpl, WallPostHandler}
...
case class Bot[F[_]: Sync](httpClient: Client[F],
  ...
  private val distanceCalculator = new DistanceCalculatorImpl
  ...
  def whenNewSearch(message: MessageNew)(thing: String, userPos: Coord): F[Unit] =
    for {
      offers <- offerRepository.selectByTType(thing).map { offs =>
        ...
        offs
          .filter(_.price.exists(x => x >= from && x <= to))
          .map { offer =>
            val geo = for {
              lat <- offer.latitude
              lon <- offer.longitude
            } yield Coord(lat, lon)
            geo.map(x => x.copy(distToUser = Some(distanceCalculator.calculateDistanceInKilometer(x, userPos))))
          }
      }
      _ <- offers match {
        case Nil =>
